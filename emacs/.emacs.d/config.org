...
However, it is also in the Melpa repositories, so can be installed using
...
A useless but fun add-in. Start the fire using M-x fireplace. The
fireplace can be controlled using:
...
*** Default Search Mode Using Regular Expressions.

One of the arcane areas of Emacs is the use of regular expressions for
things like searching or find and replace. By default the standard
search is accessed using C-s, and regular expression search is C-M-s
(using r instead of s reverses the order of the search). Given how
powerful regular expression searching is, it is worth using it as the
default.
...
**** Testing Using Hippie Expand Alternative for Dynamic Expansion.

As I have been building this configuration I have seen a couple of
pages that have very strongly 'bigged up' the use of hippie
expand. Previous testing seemed to indicate that it didn't work, but
that may be because it wasn't appropriately configured, so I'm trying
again.

Importantly, the dynamic expansion shortcut stay the same, so you
still use M-/ so there is no need to move to a different keystroke.

#+begin_src emacs-lisp
  ;; Hippie Expand Configuration.
  (global-set-key (kbd "M-/") 'hippie-expand)
  ;; Set the expansions sources to try.
...
or GoogleDrive. Unfortunately it also had what felt like a hefty price
tag.
...
reasonable sum. It is important to highlight that they are simplistic
compared to the Freewrite, having to built-in wifi function, and being
designed mainly to transfer files to a 'proper' computer by being
connected to the computer using an USB cable at which point the
Alphasmart Neo2 becomes an external keyboard, typing the text again
into a suitably provided document on the computer. This approach is
fraught with many problems because of the temptation to forget that
the typing process is taking place, and move to a different window,
which will continue to receive the output that should have been placed
in the relevant text file. The other issue that I had with my Neo2 is
far more arcane and related to the fact that I'm a Dvorak keyboard
layout user. Neo2 can be switched to use a Dvorak layout, which is
extremely useful for me as I can touchtype using Dvorak, but not
Qwerty. The Dvorak in question isn't perfect for my use as I usually
use the UK Punctuation variation of Dvorak provided on Linux (includes
the £ sign), with a swapped Caps Lock and left hand Ctrl key pair, but
it is close enough. However, when using the text transference method
where the typing is repeated, the Dvorak layout had another weird
effect. The way that the external typing worked was to send the key
position for the character that it wanted to send, not the
character. So the a and m keys are the same on both layouts, but
everything else is different. I initially started working around this
by using the Linux 'tr' command which can be used to process text
using a 1-2-1 character mapping. However, I then discovered that there
is an open source project that provides a tool for hoovering the text
files from the Neo2 as text, and therefore the Dvorak doesn't end up
being an issue. This is done using a Python3 script 'neotools'. May
add more details on this when I next use it.
...
I often end up googling for the format for Makefile files, so here is
a quick overview of the format.

Consider the situation where I have a source file called hello.c (the
standard 'hello world!' program) which I want to compile to create a
binary called hello. The entry for Makefile requires a label for the
compilation process, followed by a colon, then the compilation
command. The simplest entry would be:

hello:
      gcc hello.c -o hello

The most common addition to the command for the compilation is the
flag to generate and show any warnings that arise.

hello:
      gcc -Wall hello.c -o hello

The advantage of using this Makefile approach is that you can then run
the compilation using the compile command within Emacs as follows:

M-x compile

This will generate the entry:

make -k

and the label text can then be added to the end of the command. Again,
in this case:

make -k hello

A second pane will open to display the output of the compilation
process. Running the code can either be done from a terminal or by
opening 'eshell' to create a terminal without leaving Emacs.

There are lots of other things that can be done using the Makefile,
such as cleaning files that aren't required or are left over from
previous compilation processes.

What I don't know is how to force a recompilation when the source file
is updated. A google indicates that adding -B switch causes a full
rebuild.
*** Lisp/Clojure.

I need to add some configuration for Lisp editing other than Clojure,
but for now, I am adding some functionality as provided by the Clojure
configuration guide. I'm not using the whole thing because I have this
carefully crafted configuration in place, but will appropriate
sections. So far this has included setting up the use of
hippie-expand. I'm still not convinced by the hippie-expand
functionality in comparison to dabbrev, but for now it stays.

**** Switch on Paredit Mode Support When Working with Lisps.

One of the things that people know about Lisp is that it is very
heavily dependent on the use of parentheses. To help working with the
bracket laden code, Paredit Mode allows you to slurp or barf items
into and out of a set of brackets. Details are available in the
...
When you are coding, it is undoubtedly useful to have relevant
information displayed in the minibuffer.

#+begin_src emacs-lisp
  ;; ElDoc information in minibuffer
  (add-hook 'emacs-lisp-mode-hook 'turn-on-eldoc-mode)
  (add-hook 'lisp-interaction-mode-hook 'turn-on-eldoc-mode)
  (add-hook 'ielm-mode-hook 'turn-on-eldoc-mode)
#+end_src

Clojure support. This is an incomplete set of functions as outlined in
...
Python is a language that relies on layout more than many others, so
it is useful to have linting for the language.

#+begin_src emacs-lisp
    ;; Use Elpy mode to manage Python coding.
    (use-package elpy)
    (elpy-enable)
    (setq python-shell-interpreter "ipython3"
  	python-shell-interpreter-args "-i --simple-prompt")
    ;; Use flycheck to check the syntax as the code is written.
    (use-package flycheck)
    (when (require 'flycheck nil t)
      (setq elpy-modules (delq 'elpy-module-flymake elpy-modules))
      (add-hook 'elpy-mode-hook 'flycheck-mode))
    ;; Enable code formatting using PEP8
...
See the Makefile section for help with using Makefile to manage
